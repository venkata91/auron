...
impl JavaClasses<'static> {
    pub fn init(env: &JNIEnv) {
        if let Err(err) = JNI_JAVA_CLASSES.get_or_try_init(|| -> Result<_, Box<dyn Error>> {
            ...
            #[cfg(feature = "flink")]
            log::info!("==> FLINK MODE: Spark/Scala classes will be skipped");
            #[cfg(not(feature = "flink"))]
            log::info!("==> SPARK MODE: Loading all Spark/Scala classes");

            log::info!("Loading JniBridge...");
            let jni_bridge = JniBridge::new(env)?;
            log::info!("Loading classloader...");
            let classloader = env
                ...
                .l()?;
            log::info!("Getting JVM...");
            let jvm = env.get_java_vm()?;
            log::info!("Getting global classloader ref...");
            let classloader = get_global_ref_jobject(env, classloader)?;

            log::info!("Loading cClass...");
            let c_class = JavaClass::new(env)?;
            log::info!("Loading cJavaThrowable...");
            let c_java_throwable = JavaThrowable::new(env)?;
            log::info!("Loading cJavaRuntimeException...");
            let c_java_runtime_exception = JavaRuntimeException::new(env)?;
            log::info!("Loading cJavaReadableByteChannel...");
            let c_java_readable_byte_channel = JavaReadableByteChannel::new(env)?;
            log::info!("Loading cJavaBoolean...");
            let c_java_boolean = JavaBoolean::new(env)?;
            log::info!("Loading cJavaLong...");
            let c_java_long = JavaLong::new(env)?;
            log::info!("Loading cJavaAutoCloseable...");
            let c_java_auto_closeable = JavaAutoCloseable::new(env)?;
            log::info!("Loading cJavaURI...");
            let c_java_uri = JavaURI::new(env)?;
            log::info!("Loading cJavaBuffer...");
            let c_java_buffer = JavaBuffer::new(env)?;

            #[cfg(not(feature = "flink"))]
            log::info!("Loading Scala classes...");
            #[cfg(not(feature = "flink"))]
            let c_scala_iterator = ScalaIterator::new(env)?;
            #[cfg(not(feature = "flink"))]
            let c_scala_tuple2 = ScalaTuple2::new(env)?;
            #[cfg(not(feature = "flink"))]
            let c_scala_function0 = ScalaFunction0::new(env)?;
            #[cfg(not(feature = "flink"))]
            let c_scala_function1 = ScalaFunction1::new(env)?;
            #[cfg(not(feature = "flink"))]
            let c_scala_function2 = ScalaFunction2::new(env)?;

            log::info!("Loading cHadoopFileSystem...");
            let c_hadoop_file_system = HadoopFileSystem::new(env)?;
            log::info!("Loading cHadoopPath...");
            let c_hadoop_path = HadoopPath::new(env)?;

            #[cfg(not(feature = "flink"))]
            log::info!("Loading Spark classes...");
            #[cfg(not(feature = "flink"))]
            let c_spark_file_segment = SparkFileSegment::new(env)?;
            #[cfg(not(feature = "flink"))]
            let c_spark_sql_metric = SparkSQLMetric::new(env)?;
            #[cfg(not(feature = "flink"))]
            let c_spark_metric_node = SparkMetricNode::new(env)?;
            #[cfg(not(feature = "flink"))]
            let c_spark_auron_udf_wrapper_context = SparkAuronUDFWrapperContext::new(env)?;
            #[cfg(not(feature = "flink"))]
            let c_spark_udaf_wrapper_context = SparkUDAFWrapperContext::new(env)?;
            #[cfg(not(feature = "flink"))]
            let c_spark_udtf_wrapper_context = SparkUDTFWrapperContext::new(env)?;
            #[cfg(not(feature = "flink"))]
            let c_spark_udaf_mem_tracker = SparkUDAFMemTracker::new(env)?;
            #[cfg(not(feature = "flink"))]
            let c_auron_conf = AuronConf::new(env)?;
            #[cfg(not(feature = "flink"))]
            let c_auron_rss_partition_writer_base = AuronRssPartitionWriterBase::new(env)?;

            log::info!("Loading cAuronCallNativeWrapper...");
            let c_auron_call_native_wrapper = AuronCallNativeWrapper::new(env)?;
            ...
            log::info!("Loading cAuronArrowFFIExporter...");
            let c_auron_arrow_ffi_exporter = AuronArrowFFIExporter::new(env)?;
            log::info!("Loading cAuronFSDataInputWrapper...");
            let c_auron_fs_data_input_wrapper = AuronFSDataInputWrapper::new(env)?;
            log::info!("Loading cAuronFSDataOutputWrapper...");
            let c_auron_fs_data_output_wrapper = AuronFSDataOutputWrapper::new(env)?;

            #[cfg(not(feature = "flink"))]
            let c_auron_json_fallback_wrapper = AuronJsonFallbackWrapper::new(env)?;

            log::info!("Loading cAuronUDFWrapperContext...");
            let c_auron_udf_wrapper_context = AuronUDFWrapperContext::new(env)?;

            log::info!("Assembling JavaClasses struct...");
            let java_classes = JavaClasses {
                ...
                cClass: c_class,
                cJavaThrowable: c_java_throwable,
                cJavaRuntimeException: c_java_runtime_exception,
                cJavaReadableByteChannel: c_java_readable_byte_channel,
                cJavaBoolean: c_java_boolean,
                cJavaLong: c_java_long,
                cJavaAutoCloseable: c_java_auto_closeable,
                cJavaURI: c_java_uri,
                cJavaBuffer: c_java_buffer,

                #[cfg(not(feature = "flink"))]
                cScalaIterator: c_scala_iterator,
                #[cfg(not(feature = "flink"))]
                cScalaTuple2: c_scala_tuple2,
                #[cfg(not(feature = "flink"))]
                cScalaFunction0: c_scala_function0,
                #[cfg(not(feature = "flink"))]
                cScalaFunction1: c_scala_function1,
                #[cfg(not(feature = "flink"))]
                cScalaFunction2: c_scala_function2,

                cHadoopFileSystem: c_hadoop_file_system,
                cHadoopPath: c_hadoop_path,
                ...
                #[cfg(not(feature = "flink"))]
                cSparkMetricNode: c_spark_metric_node,
                #[cfg(not(feature = "flink"))]
                ...
                cAuronArrowFFIExporter: c_auron_arrow_ffi_exporter,
                cAuronFSDataInputWrapper: c_auron_fs_data_input_wrapper,
                cAuronFSDataOutputWrapper: c_auron_fs_data_output_wrapper,
                #[cfg(not(feature = "flink"))]
                cAuronJsonFallbackWrapper: c_auron_json_fallback_wrapper,

                cAuronUDFWrapperContext: c_auron_udf_wrapper_context,
            };
